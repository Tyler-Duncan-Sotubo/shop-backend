...
import {
  FinalizeBankTransferPaymentInput,
...
function assertEvidenceMime(mimeType: string) {
...
export class PaymentService {
  ...
  async listPayments(companyId: string, filter: ListPaymentsQueryInput) {
  ...
  /**
   * Storefront/Admin: presign an evidence upload (client uploads to S3).
   * - Validates payment exists & belongs to company.
   * - Optionally enforce bank_transfer + pending for storefront usage.
   */
  async presignPaymentEvidenceUpload(params: {
    ...
    publicRead?: boolean;
    // if true, only allow pending bank transfer to upload evidence
    ...
    const {
      ...
      publicRead = true,
      requirePendingBankTransfer = false,
    } = params;
    ...
    // Ensure payment exists + belongs to company
    ...
    if (requirePendingBankTransfer) {
      ...
      if (p.status !== 'pending') {
        throw new BadRequestException(
          'Evidence can only be uploaded for pending payments',
        );
      }
    }
    ...
    const key = `companies/${companyId}/payments/${paymentId}/evidence/tmp/${defaultId()}-${finalName}`;
    ...
    // Make sure the client knows the key to send back on finalize
    return {
      upload: {
        ...presign,
        key,
  ...
  /**
   * Storefront/Admin: finalize evidence after client uploads.
   * - HEADs object to get size/content-type
   * - Inserts into payment_files
   */
  async finalizePaymentEvidenceUpload(params: {
    ...
    // HEAD to verify object exists + derive metadata
    ...
    const [row] = await this.db
      ...
      .values({
        ...
        fileName: safeName,
        mimeType: finalMime,
        sizeBytes: sizeBytes as any, // if your column is sizeBytes
