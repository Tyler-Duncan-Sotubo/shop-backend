...
let CategoriesService = class CategoriesService {
    ...
    async createCategory(companyId, dto, user, ip) {
        ...
        const created = await this.db.transaction(async (tx) => {
            const [category] = await tx
                ...
                .returning()
                .execute();
            const imageMediaId = dto.imageMediaId;
            const base64Image = dto.base64Image;
            if (imageMediaId && base64Image) {
                throw new common_1.BadRequestException('Provide either imageMediaId or base64Image, not both.');
            }
            ...
            if (finalMediaId) {
                const [updated] = await tx
                    ...
                    .set({
                    ...(schema_1.categories.imageMediaId
    ...
    async updateCategory(companyId, categoryId, dto, user, ip) {
        ...
        const updated = await this.db.transaction(async (tx) => {
            ...
            const imageMediaId = dto.imageMediaId;
            const base64Image = dto.base64Image;
            const removeImage = dto.removeImage;
            if (imageMediaId && base64Image) {
                throw new common_1.BadRequestException('Provide either imageMediaId or base64Image, not both.');
            }
            if (removeImage === true) {
