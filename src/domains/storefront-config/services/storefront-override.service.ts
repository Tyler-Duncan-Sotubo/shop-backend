...
import { storefrontOverrides, stores } from 'src/infrastructure/drizzle/schema';
...
export class StorefrontOverrideService {
  ...
  async getStorefrontOverrideStatus(companyId: string, storeId: string) {
    const store = await this.db.query.stores.findFirst({
      where: eq(stores.id, storeId),
    });
    if (!store) throw new NotFoundException('Store not found');
    ...
    return {
      ...
      // handy computed flags for UI
      isLive: !!published?.themeId, // “we have a published theme”
      hasDraftChanges: !!draft, // “draft override exists”
