...
import { paymentMethods, stores } from 'src/infrastructure/drizzle/schema';
import { CompanySettingsService } from 'src/domains/company-settings/company-settings.service';
import {
  PaymentMethodType,
  ToggleStorePaymentMethodInput,
  UpsertBankTransferConfigInput,
...
export class PaymentMethodsService {
  ...
  async getCheckoutMethods(companyId: string, storeId: string) {
    ...
    const gateway = rows
      .filter((r: any) => r.method === 'gateway' && r.isEnabled)
      .map((r: any) => ({
        method: 'gateway',
        provider: r.provider,
      ...
      methods: [
        ...
        ...(cash?.isEnabled
          ? [
              {
                method: 'cash',
                // optional public config (label, note)
                note:
                  (cash.config as any)?.note ??
  ...
  async toggle(companyId: string, dto: ToggleStorePaymentMethodInput) {
  ...
  async upsertGateway(companyId: string, dto: UpsertGatewayConfigInput) {
  ...
  async upsertBankTransfer(
    companyId: string,
    dto: UpsertBankTransferConfigInput,
  ) {
