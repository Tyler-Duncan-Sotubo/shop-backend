// src/domains/iam/permissions/company-access.service.ts
import {
  BadRequestException,
  Inject,
  Injectable,
...
export class CompanyAccessService {
    ...
    const existingRole = await this.db
      ...
      .where(
        and(
          ...
          eq(companyRoles.name, name as any),
        ),
      )
      .execute();
   ...
   * Canonical role set: owner, manager, staff, support.
   */
  async createDefaultRoles(companyId: string) {
    ...
    const insertedRoles = await this.db
      .insert(companyRoles)
      .values(defaultRoles.map((name) => ({ companyId, name: name as any })))
      .returning()
      .execute();
    ...
    return this.cache.getOrSetVersioned(companyId, ['roles'], async () => {
      return this.db
        .select({
          ...
          displayName: companyRoles.displayName,
        })
        .from(companyRoles)
  ...
  public async seedDefaultPermissionsForCompany(
    ...
    for (const r of existingRows) alreadySet.add(`${r.roleId}|${r.permId}`);

    const allPermissions = await this.db.select().from(permissions).execute();
    ...
    for (const role of roles) {
      const permittedKeys = (DefaultRolePermissions as any)[role.name] || [];
  ...
  async createCompanyRole(params: {
    ...
    // clone permissions from base role (optional)
  ...
  private async cloneRolePermissions(
    sourceRoleId: string,
    targetRoleId: string,
  ) {
    const rows = await this.db
      .select({ permissionId: companyRolePermissions.permissionId })
      .from(companyRolePermissions)
      .where(eq(companyRolePermissions.companyRoleId, sourceRoleId))
      .execute();
