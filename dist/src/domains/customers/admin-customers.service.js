    ...
    WITH people AS (
      ${includeCustomers
            ...
            SELECT
              ...
              CASE
                WHEN ${schema_1.customers.marketingOptIn} THEN 'subscribed'
                ELSE 'unsubscribed'
            ...
            WHERE ${schema_1.customers.companyId} = ${companyId}
              ...
              ${s
                ? (0, drizzle_orm_1.sql) `
                    AND (
                      ${schema_1.customers.displayName} ILIKE ${'%' + s + '%'}
                      OR ${schema_1.customers.firstName} ILIKE ${'%' + s + '%'}
                      OR ${schema_1.customers.lastName} ILIKE ${'%' + s + '%'}
                      OR ${schema_1.customers.billingEmail} ILIKE ${'%' + s + '%'}
                      OR ${schema_1.customerCredentials.email} ILIKE ${'%' + s + '%'}
                      OR ${schema_1.customers.phone} ILIKE ${'%' + s + '%'}
                    )
                  `
                : (0, drizzle_orm_1.sql) ``}
          `
      ...
      ${includeCustomers && includeSubscribers ? (0, drizzle_orm_1.sql) `UNION ALL` : (0, drizzle_orm_1.sql) ``}
