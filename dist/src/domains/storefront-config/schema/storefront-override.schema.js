"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.StorefrontOverridesV1Schema = exports.PaymentMethods = void 0;
const zod_1 = require("zod");
exports.PaymentMethods = [
    'visa',
    'mastercard',
    'verve',
    'amex',
    'discover',
    'paypal',
    'apple_pay',
    'google_pay',
    'bank_transfer',
];
const PaymentMethodEnum = zod_1.z.enum(exports.PaymentMethods);
const PaymentsOverrideSchema = zod_1.z
    .object({
    enabled: zod_1.z.boolean().optional(),
    methods: zod_1.z.record(PaymentMethodEnum, zod_1.z.boolean()).optional(),
})
    .strict();
exports.StorefrontOverridesV1Schema = zod_1.z
    .object({
    ui: zod_1.z
        .object({
        headerMenu: zod_1.z
            .object({
            about: zod_1.z.boolean().optional(),
            contact: zod_1.z.boolean().optional(),
            blog: zod_1.z.boolean().optional(),
        })
            .strict()
            .optional(),
    })
        .strict()
        .optional(),
    theme: zod_1.z
        .object({
        assets: zod_1.z
            .object({
            logoUrl: zod_1.z.string().url().optional(),
        })
            .strict()
            .optional(),
    })
        .passthrough()
        .optional(),
    seo: zod_1.z
        .object({
        title: zod_1.z.string().optional(),
        description: zod_1.z.string().optional(),
        favicon: zod_1.z
            .object({
            ico: zod_1.z.string().optional(),
            png: zod_1.z.string().optional(),
            svg: zod_1.z.string().optional(),
            appleTouch: zod_1.z.string().optional(),
        })
            .optional(),
        ogImage: zod_1.z
            .object({
            url: zod_1.z.string().optional(),
            alt: zod_1.z.string().optional(),
        })
            .optional(),
    })
        .optional(),
    header: zod_1.z
        .object({
        topBar: zod_1.z
            .object({
            enabled: zod_1.z.boolean().optional(),
            autoplay: zod_1.z
                .object({
                enabled: zod_1.z.boolean().optional(),
                intervalMs: zod_1.z.number().int().min(3000).max(60000).optional(),
            })
                .strict()
                .optional(),
            slides: zod_1.z.array(zod_1.z.object({ text: zod_1.z.string().min(1) })).optional(),
        })
            .strict()
            .optional(),
        nav: zod_1.z
            .object({
            enabled: zod_1.z.boolean().optional(),
            items: zod_1.z
                .array(zod_1.z
                .object({
                label: zod_1.z.string().min(1),
                href: zod_1.z.string().min(1),
            })
                .strict())
                .optional(),
            icons: zod_1.z
                .object({
                search: zod_1.z.boolean().optional(),
                account: zod_1.z.boolean().optional(),
                wishlist: zod_1.z.boolean().optional(),
                cart: zod_1.z.boolean().optional(),
                quote: zod_1.z.boolean().optional(),
            })
                .strict()
                .optional(),
        })
            .strict()
            .optional(),
    })
        .strict()
        .optional(),
    footer: zod_1.z
        .object({
        brand: zod_1.z
            .object({
            logoUrl: zod_1.z.string().url().optional(),
            blurb: zod_1.z.string().optional(),
        })
            .optional(),
        newsletter: zod_1.z
            .object({
            enabled: zod_1.z.boolean().optional(),
            title: zod_1.z.string().optional(),
            description: zod_1.z.string().optional(),
            placeholder: zod_1.z.string().optional(),
            ctaLabel: zod_1.z.string().optional(),
        })
            .optional(),
        bottomBar: zod_1.z
            .object({
            leftText: zod_1.z.string().optional(),
            payments: PaymentsOverrideSchema.optional(),
        })
            .optional(),
    })
        .optional(),
    pages: zod_1.z
        .object({
        home: zod_1.z
            .object({
            hero: zod_1.z
                .object({
                enabled: zod_1.z.boolean().optional(),
                image: zod_1.z
                    .object({
                    src: zod_1.z.string().min(1).optional(),
                    alt: zod_1.z.string().optional(),
                })
                    .optional(),
                content: zod_1.z
                    .object({
                    eyebrow: zod_1.z.string().optional(),
                    heading: zod_1.z.string().optional(),
                    description: zod_1.z.string().optional(),
                    cta: zod_1.z
                        .object({
                        label: zod_1.z.string().optional(),
                        href: zod_1.z.string().optional(),
                    })
                        .optional(),
                })
                    .optional(),
                bottomStrip: zod_1.z
                    .object({
                    enabled: zod_1.z.boolean().optional(),
                    text: zod_1.z.string().optional(),
                    className: zod_1.z.string().optional(),
                })
                    .optional(),
                variant: zod_1.z.string().optional(),
                layout: zod_1.z.record(zod_1.z.unknown()).optional(),
                overlay: zod_1.z.record(zod_1.z.unknown()).optional(),
            })
                .optional(),
            sections: zod_1.z.array(zod_1.z.unknown()).optional(),
            contact: zod_1.z.record(zod_1.z.unknown()).optional(),
        })
            .optional(),
        about: zod_1.z
            .object({
            version: zod_1.z.number().int().optional(),
            seo: zod_1.z.record(zod_1.z.unknown()).optional(),
            sections: zod_1.z.array(zod_1.z.unknown()).optional(),
        })
            .optional(),
        contact: zod_1.z
            .object({
            version: zod_1.z.number().int().optional(),
            seo: zod_1.z.record(zod_1.z.unknown()).optional(),
            sections: zod_1.z.array(zod_1.z.unknown()).optional(),
        })
            .optional(),
    })
        .optional(),
})
    .strict();
//# sourceMappingURL=storefront-override.schema.js.map