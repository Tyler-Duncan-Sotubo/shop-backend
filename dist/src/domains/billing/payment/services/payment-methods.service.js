...
let PaymentMethodsService = class PaymentMethodsService {
    ...
    assertGatewayProvider(dto) {
        if (dto.method === inputs_1.PaymentMethodType.gateway && !dto.provider) {
    ...
    async getCheckoutMethods(companyId, storeId) {
        ...
        const gateway = rows
            .filter((r) => r.method === 'gateway' && r.isEnabled)
            .map((r) => ({
            method: 'gateway',
            provider: r.provider,
    ...
    async toggle(companyId, dto) {
        ...
        const provider = dto.method === inputs_1.PaymentMethodType.gateway ? dto.provider : null;
    ...
    async upsertGateway(companyId, dto) {
        const method = inputs_1.PaymentMethodType.gateway;
        const rows = await this.db
            .select()
            .from(schema_1.paymentMethods)
    ...
    async upsertBankTransfer(companyId, dto) {
        const method = inputs_1.PaymentMethodType.bank_transfer;
        const rows = await this.db
            .select()
            .from(schema_1.paymentMethods)
