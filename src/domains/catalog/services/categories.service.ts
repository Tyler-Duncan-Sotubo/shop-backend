...
export class CategoriesService {
  ...
  async createCategory(
    ...
    // ─────────────────────────────────────────────────────────────
    // Image: either attach existing media OR finalize a presigned upload
    // ─────────────────────────────────────────────────────────────
    const {
      imageMediaId,
      uploadKey,
    ...
    if (imageMediaId && uploadKey) {
      throw new BadRequestException(
        'Provide either imageMediaId OR uploadKey/uploadUrl, not both.',
      );
    }
    ...
    if (imageMediaId) {
      ...
      if (!uploadUrl || !imageFileName || !imageMimeType) {
        throw new BadRequestException(
          'uploadKey requires uploadUrl, imageFileName, imageMimeType',
        );
      }
    ...
    const created = await this.db.transaction(async (tx) => {
      ...
      if (finalMediaId) {
        const [updated] = await tx
          .update(categories)
          .set({
            imageMediaId: finalMediaId as any,
            updatedAt: new Date(),
          })
          .where(
            and(
    ...
    if (user && ip) {
      await this.audit.logAction({
        ...
        changes: {
          ...
          imageMediaId: (created as any).imageMediaId ?? null,
        },
      });
    }
    ...
    const updated = await this.db.transaction(async (tx) => {
      const [row] = await tx
        ...
        .set({
          ...
          afterContentHtml:
            dto.afterContentHtml !== undefined
      ...
      if (removeImage === true) {
        const [row2] = await tx
          .update(categories)
          .set({ imageMediaId: null as any, updatedAt: new Date() })
          .where(
            and(
              eq(categories.companyId, companyId),
              eq(categories.id, categoryId),
      ...
      if (imageMediaId) {
        ...
        const [row2] = await tx
          .update(categories)
          .set({ imageMediaId: imageMediaId as any, updatedAt: new Date() })
          .where(
            and(
              eq(categories.companyId, companyId),
              eq(categories.id, categoryId),
      ...
      if (uploadKey) {
        ...
        const [row2] = await tx
          .update(categories)
          .set({ imageMediaId: finalized.id as any, updatedAt: new Date() })
          .where(
            and(
              eq(categories.companyId, companyId),
              eq(categories.id, categoryId),
